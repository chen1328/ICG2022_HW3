...
Skybox::Skybox(const string& texImagePath, const int nSlices, const int nStacks, const float radius)
{
	rotationY = 0.0f;

	// Load panorama.
	panorama = new ImageTexture(texImagePath);
	if (!panorama->GetSuccessLoaded())
	{
		delete panorama;
		panorama = nullptr;
		return;
...
void Skybox::Render(Camera* camera, SkyboxShaderProg* shader, const float curRotationY)
{
	glEnableVertexAttribArray(0);
	glEnableVertexAttribArray(1);

	glBindBuffer(GL_ARRAY_BUFFER, vboId);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(VertexPT), (const GLvoid*)0);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(VertexPT), (const GLvoid*)offsetof(VertexPT, texcoord));

	shader->Bind();
	
	// Set transform.
	glm::mat4x4 worldMatrix = glm::rotate(glm::mat4x4(1.0f), glm::radians(curRotationY), glm::vec3(0.0f, 1.0f, 0.0f));
							...
							vector<VertexPT>& vertices, vector<unsigned int>& indices)
{
	...
	for (int t = 0; t <= numTheta; ++t)
	{
		for (int p = 0; p <= numPhi; ++p) 
		{
			...
			// cout << glm::degrees<float>(phi) << " " << glm::degrees<float>(theta) << std::endl;
			glm::vec2 uv = glm::vec2((float)p / (float)numPhi, (float)t / (float)numTheta);
			// cout << uv.x << " " << uv.y << std::endl;
